...
def pagina(filename):
    ...
    input: templates en de pagina die aangeklikt is op de bestaande
    pagina en zoekdata als die nodig zijn
    output: weergave van de uitgekozen pagina en de data die hoort bij
    de zoekdata
    """
    if filename == "bacterie.html":
        ...
        if request.method == 'POST':
            ...
            cursor.execute("""select blast.name, blast.accessioncode, 
            functionality.function from blast join functionint on blast.id = 
            functionint.BLAST_id join functionality on 
            functionint.functionality_id = functionality.id where blast.name 
            like '%" + zoekwoord + "%'""")
            records = cursor.fetchall()  # lijst met al de namen die het zoekwoord in de naam hebben
            cursor.close()
            conn.close()
            # maakt een tabel van de gevonden data
            teruggeven = ("<table id=\"myTable\" style=\"width:777px; height: 400px;\"><tr>\n"
        ...
        return render_template("bacterie.html", teruggeven=teruggeven,
                               zoekwoord=zoekwoord)
        ...
        if request.method == 'POST':
            conn = mysql.connector.connect(host="hannl-hlo-bioinformatica-mysqlsrv.mysql.database.azure.com",
                                           ...
                                           db="kxxxf", password="ConnectionPWD")
            cursor = conn.cursor()
            ...
            #
            cursor.execute("""select blast.name, evalue, sequence.score, 
            querycoverage, percidentity from blast join sequence on 
            blast.id = sequence.id where 1=1 """ + sql)
            records = cursor.fetchall()  # lijst met al de namen die het zoekwoord in de naam hebben
            cursor.close()
            conn.close()
            teruggeven = ("<table id=\"myTable\" style=\"width:777px; "
                          + "height: 400px;\"><tr>\n"
                          + "<th onclick=\"sortTable(0)\">Naam</th>\n"
        ...
        conn = mysql.connector.connect(host="hannl-hlo-bioinformatica-mysqlsrv.mysql.database.azure.com",
                                       ...
                                       password="ConnectionPWD")
        cursor = conn.cursor()
        cursor.execute("select name, count(*) from blast group by name order "
                       "by 2 desc limit 5")
        records = cursor.fetchall()  # lijst met al de namen die het zoekwoord in de naam hebben
        cursor.close()
        conn.close()
